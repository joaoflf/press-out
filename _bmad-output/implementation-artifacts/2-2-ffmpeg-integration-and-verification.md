...
3. **Given** a test video file exists in testdata/, **When** the FFmpeg integration test runs, **Then** FFmpeg is invoked successfully on the sample video, **And** the test verifies that output is produced and is a valid output file (video or image depending on the operation)

## Tasks / Subtasks

- [ ] Create FFmpeg helper package at `internal/ffmpeg/ffmpeg.go` (AC: 2)
  - [ ] `Run(ctx context.Context, args ...string) (stdout []byte, stderr []byte, err error)` — core execution function for `ffmpeg`
  - [ ] `RunProbe(ctx context.Context, args ...string) (stdout []byte, stderr []byte, err error)` — core execution function for `ffprobe` (same pattern as `Run()` but invokes the `ffprobe` binary)
  - [ ] Use `exec.CommandContext(ctx, "ffmpeg", args...)` / `exec.CommandContext(ctx, "ffprobe", args...)` for timeout support
  ...
  - [ ] Log invocation with `slog.Info("ffmpeg exec", "binary", binary, "args", args)`
  - [ ] Log completion with `slog.Info("ffmpeg complete", "binary", binary, "duration_ms", elapsed)`

- [ ] Add FFmpeg probe function (AC: 1)
  - [ ] `Probe() (version string, err error)` — runs `ffmpeg -version` and parses version string, also verifies `ffprobe` is available
...
- [ ] Add FFmpeg convenience functions for common operations (AC: 2)
  ...
  - [ ] `GetDuration(ctx context.Context, input string) (float64, error)` — get video duration in seconds via `RunProbe()`
  - [ ] All convenience functions MUST include the `-y` flag to allow overwriting existing output files (required for pipeline re-trigger in Story 2.5)
  - [ ] Each function builds the correct argument list and calls `Run()` (or `RunProbe()` for metadata queries)
  - [ ] `CropVideo` parameter order is `x, y, w, h` — the function must map these to FFmpeg's crop filter format `crop=w:h:x:y` internally
...
- **FFmpeg and ffprobe must be installed** on the development machine before this story can be implemented or tested. Both binaries ship together.
  - macOS: `brew install ffmpeg`
  - Linux (Debian/Ubuntu): `sudo apt install ffmpeg`
  - Verify: `ffmpeg -version && ffprobe -version`
- A `make check-deps` target should be added to verify FFmpeg availability and fail with clear instructions if missing.
...
- `ffprobe` (installed alongside FFmpeg) is used for video metadata queries like duration via `RunProbe()`. Same execution pattern as FFmpeg but invokes a separate binary.

### Architecture Compliance

- FFmpeg helper goes in `internal/ffmpeg/` — a new package extending the architecture's package list, consistent with the `internal/mediapipe/` and `internal/claude/` pattern for external tool integration. The architecture doc should be updated to include this package
