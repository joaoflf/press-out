...
3. **Given** the lifter taps into a processing lift, **When** the lift detail page loads, **Then** a full vertical stage checklist is displayed with 6 stages (Trimming, Cropping, Pose estimation, Rendering skeleton, Computing metrics, Generating coaching) with three states per stage (pending: dimmed, active: pulsing dot, complete: sage checkmark), **And** the checklist updates in real-time via SSE as stages complete
...
- [ ] Define Stage interface and types in `internal/pipeline/stage.go` (AC: 1, 4)
  ...
  - [ ] Stage names as constants: `StageTrimming`, `StageCropping`, `StagePoseEstimation`, `StageRenderingSkeleton`, `StageComputingMetrics`, `StageGeneratingCoaching`
...
- [ ] Create pipeline stages template `web/templates/partials/pipeline-stages.html` (AC: 2, 3)
  ...
  - [ ] Compact variant (list view): single line showing current stage + "N of 6"
