...
4. Server-side pose estimation via MediaPipe
...
**Technical Risk:** Pose estimation quality on real gym video (occlusion, lighting, bystanders) is the highest-risk component. No fallback — the product is contingent on MediaPipe producing usable keypoints. Mitigation: test early with real gym footage, accept degraded skeleton quality over failure, and tune confidence thresholds for noisy conditions.
